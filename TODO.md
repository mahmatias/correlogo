...
- [ ] **8-c esteira: validar telemetria real no device (P7)** — ✅ implementado 2026-09-04c (CHANGELOG): points.speedKmh usam velocidade REAL reportada (`metrics.instantSpeedKmh`), auto-pause por velocidade reportada, instrumentação acumulada + log `[telemetry] resumo esteira:` no save. **Aguardando teste no device** com a esteira real para: 1) conferir se o dado FTMS é confiável (P7) e calibrar B/B2/B-a; 2) validar auto-pause no joystick real (vel < 1.0 por 5s pausa, > 3.0 resume)
