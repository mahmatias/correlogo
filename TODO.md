...
### Alta (imediato)
- [ ] **Testar no device — 3 correções 2026-08-28** — (1) **TTS/AudioFocus**: falar 2+ comandos no mesmo instante não deve cortar o segundo, e a música deve restaurar o volume ao final; (2) **Strava auto-sync**: salvar relatório deve enviar o email automático e badge fica verde/`synced` sem clique manual (se falhar, ver `[strava] auto-send rejected:` ou `[strava] send failed:` no logcat — causa do reject ainda [Guessing]); (3) **CARTO**: mapa sem watermark "API KEY REQUIRED" em **treino, resumo E card de compartilhamento** (card-map agora com key + secret `ENV_FILE` atualizado na CI — ver CHANGELOG `2026-08-28b`)
