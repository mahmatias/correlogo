...
### Alta (imediato)
- [ ] **Rotação de credenciais — vazamento 2026-07-30 (SECURITY, quase concluído)** — auditado e remediado 2026-08-16 (ver CHANGELOG 2026-08-16). Estado: **SA key rotacionada** ✅; **keystore rotacionado** ✅; **purge do branch `mahmatias-patch-1`** (`9eaa9f5→e6834d2`) ✅; **`WEB_CLIENT_SECRET` rotacionado** ✅ (novo no Secret Manager v1 + functions deployadas + Calendar/Gmail testados + secret vazado `GOCSPX-eaxIk…` desabilitado/deletado no Console). **Faltam (ação do usuário)**: (1) Reset secret do client órfão `550159999478` (mata `GOCSPX-gw4d5…`) e limpar `.env.dev` (`GOOGLE_CLIENT_SECRET`, `VITE_GOOGLE_CLIENT_ID`); (2) ticket suporte GitHub p/ purgar commit `4c3afa07` do object store + `refs/pull/1/head` (blob por SHA e PR ref continuam expostos); (3) commit/push das mudanças de código (functions + TODO + CHANGELOG + HANDOFF) — o secret já está no Secret Manager, deploy das functions já feito
