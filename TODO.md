...
### Alta (imediato)
- [x] **Web `correlogo.web.app` quebrada — `Cannot access 'Dt' before initialization`** — ✅ RESOLVIDO 2026-09-04b: a web servia bundle antigo (`C6_C6ut3`) com bug TDZ; deploy manual do build `42b5352` (`firebase deploy --only hosting:correlogo`) corrigiu. Ver CHANGELOG `2026-09-04b`
- [x] **8-c esteira: validar telemetria real no device (P7)** — ✅ implementado 2026-09-04c (CHANGELOG): points.speedKmh usam velocidade REAL reportada (`metrics.instantSpeedKmh`), auto-pause por velocidade reportada, instrumentação acumulada + log `[telemetry] resumo esteira:` no save. **✅ VALIDADO** pelo usuário (dados via Bluetooth conferem com a esteira real)
- [ ] **Web: CI não publica o site — pendência de infra** — o workflow `firebase-deploy.yml` só builda o APK; a web só muda por deploy manual. Decisão do usuário (2026-09-04b): **não** automatizar por enquanto. Risco: build web quebrado = web fica no bundle velho até deploy manual
...
- [x] **AGENTS.md desatualizado** — ✅ superado 2026-09-12: seção reescrita em "Production & Deploy — Firebase Only" (AWS/EC2/PM2 removidos) + nota do release `latest` recriado a cada CI
...
- [ ] **BLE: medir distância do BLE vs cálculo próprio** — ✅ endereçado em 2026-09-12 (CHANGELOG): estratégia **híbrida** (`HybridDistance` em `src/lib/treadmill-distance.ts`) — usa o odômetro FTMS quando presente e monotônico; fallback para integração `v×dt` (velocidade real) se zerar/regredir. Falta só validar no device que o odômetro é monotônico em frames de 1s
...
- [ ] **Reteste Gmail no device (build 177+)** — aguardando push em `main` → CI → release `latest`. Aceite: exportar Strava >1h após conectar **sem re-login** (se re-login semanal voltar, publicar o consent screen — refresh de apps em modo Testing expira em 7 dias) + navegação back fechando modais
